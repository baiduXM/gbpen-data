...
// 产品展示
    if ($(".case-list img").length > 1) {
        $(".case").hover(function() {
            $(".case .scroll").find('a.prev').stop(true, true).animate({left:"7px"}, 300).siblings('a.next').stop(true, true).animate({right:"7px"}, 300);
        }, function() {
            $(".case .scroll").find('a.prev').animate({left:"-50px"}, 300).siblings('a.next').animate({right:"-50px"}, 300);
        });
    };
