...
$(document).ready(function () {
	...
	$('#sub_container,#sub_pro_bg').delay(1000).fadeIn(1000)
	$(".sidebar").delay(1500).animate({"left":"0"},1200)
		$(".main").delay(1500).animate({"right":"-5"},1000)
		});
...
<!--记住多个的内容都不一样的话就要用遍历-->
$(".in_h2 .title").each(function() {
    
str= $(this).text(); 
str2 = str.substr(0,1);//取出字符串前俩个字符。
str3="<b>"+str2+"</b>"; //给取出字符串前俩个字符处理加上标签。 
str = str.replace(str2,str3)//将处理后的替换没有处理的。 
$(this).html(str);//最后添加到原来的字符中
...
<!--关于我们-->
 $(".about").each(
 function(){
	 $(this).find(".about_text").width($(this).width()-$(this).find(".about_img img").width()-40)
	 }
 )
...
<!--新闻中心-->
 ...
 $(".news").each(
 function(){
	 $(this).find(".news_text").width($(this).width()-$(this).find(".news_img img").width()-20)
	 }
 ) 
...
/*首页产品列表图片自适应*/
adaption(".products .inner ul li .pic",150,150)
