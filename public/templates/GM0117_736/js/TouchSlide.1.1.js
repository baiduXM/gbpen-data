var TouchSlide = function(aV) {
    ...
    aQ = function(d, c) { ! d || !c || d.className && -1 != d.className.search(new RegExp("\\b" + c + "\\b")) || (d.className += (d.className ? " ": "") + c)
    },
    aP = function(d, c) { ! d || !c || d.className && -1 == d.className.search(new RegExp("\\b" + c + "\\b")) || (d.className = d.className.replace(new RegExp("\\s*\\b" + c + "\\b", "g"), ""))
    },
    aO = aU.effect,
    ...
    var aj, ai, aJ = aK.children.length,
    aI = aS(aU.titCell, aT),
    aH = aI ? aI.length: aJ,
    aG = aU.switchLoad,
    aF = parseInt(aU.defaultIndex),
    aE = parseInt(aU.delayTime),
    aD = parseInt(aU.interTime),
    aC = "false" == aU.autoPlay || 0 == aU.autoPlay ? !1 : !0,
    aB = "false" == aU.autoPage || 0 == aU.autoPage ? !1 : !0,
    aA = "false" == aU.pnLoop || 0 == aU.pnLoop ? !1 : !0,
    az = aF,
    ay = null,
    ax = null,
    av = null,
    ...
    ar = 0,
    aq = 0,
    ap = /hp-tablet/gi.test(navigator.appVersion),
    ao = "ontouchstart" in window && !ap,
    an = ao ? "touchstart": "mousedown",
    am = ao ? "touchmove": "",
    al = ao ? "touchend": "mouseup",
    ak = aK.parentNode.clientWidth,
    ah = aJ;
    if (0 == aH && (aH = aJ), aB) {
        aH = aJ,
        aI = aI[0],
    ...
    "leftLoop" == aO && (ah += 2, aK.appendChild(aK.children[0].cloneNode(!0)), aK.insertBefore(aK.children[aJ - 1].cloneNode(!0), aK.children[0])),
    ...
    for (var af = 0; ah > af; af++) {
    	// if($(".tempWrap").width != ak){
    	// 	alert($(".tempWrap").width);
    	// 	alert(ak);
    	// }
    ...
    L = function(b) {
        ...
        0 == aA && (aP(aM, "nextStop"), aP(aN, "prevStop"), 0 == aF ? aQ(aN, "prevStop") : aF == aH - 1 && aQ(aM, "nextStop")),
    ...
    aM && aM.addEventListener("click",
    function() { (1 == aA || aF != aH - 1) && (clearTimeout(ax), clearTimeout(av), aF++, L())
    }),
    aN && aN.addEventListener("click",
    function() { (1 == aA || 0 != aF) && (clearTimeout(ax), clearTimeout(av), aF--, L())
    });
    var F = function(d) {
        clearTimeout(ax),
        clearTimeout(av),
