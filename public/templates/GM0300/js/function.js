...
$("section .wrapper").each(function(){
	...
	str= $(this).find(".aboutboxtop .bg .in_en").text(); 
    str2 = str.substr(0,1);//取出字符串前俩个字符。
    str3="<span>"+str2+"</span>"; //给取出字符串前俩个字符处理加上标签。 
    str = str.replace(str2,str3)//将处理后的替换没有处理的。 
    $(this).find(".aboutboxtop .bg .in_en").html(str);//最后添加到原来的字符中
									
									
	var tet=$(this).find(".aboutboxtop .math").text()
   tet=tet.substr(0,1); 
   $(this).find(".aboutboxtop .math").text(tet)								
...
$("#in_text ul li .in_data").height($("#in_text ul li .in_textbox").height()-20)
