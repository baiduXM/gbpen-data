 $(document).ready(function(){
	...
	jQuery(".picMarquee-left").slide({mainCell:".bd ul",autoPlay:true,effect:"leftMarquee",vis:5,interTime:50});
		...
		//大图轮播
 		jQuery(".slideBox1").slide({ titCell:".hd ul", mainCell:".bd ul", effect:"left", delayTime:1000, autoPlay:true, autoPage:true, trigger:"click",
		mouseOverStop:false,/* 鼠标移到容器层继续播放*/
		/* 控制进度条 */
			startFun:function(){
				var timer = jQuery(".slideBox1 .timer");
				timer.stop(true,true).animate({ "width":"0%" },0).animate({ "width":"100%" },1000);
			}
		});
 		//内页轮播
	...
	(function(){
			$('.sect1 .search .txt1').focus(function(){
				if($(this).val()=='请输入搜索关键字'){
					$('.sect1 .search .txt1').val('');
				}
		});
		$('.sect1 .search .txt1').blur(function(){
			if($(this).val()==''){
				$('.sect1 .search .txt1').val('请输入搜索关键字')
			}
		});			
	})();
 });