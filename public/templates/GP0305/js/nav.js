	...
	for(var i=0;i<oUl.children.length;i++)
	{
		...
		oUl.children[i].onmouseover=function()
		{
			...
			// console.log($(this).height());
		  	...
		  	{	
		  		$(this).addClass("cu");
		  		$(this).parents().find('.green').stop(true,true).slideDown(0);
		  		// $(this).parents().find('.green').stop(true,true).slideDown(400);
		  		//取出二级ul
				...
				if (liLeft>cont)
				{
					if ((sum/2) >= (oNav.offsetWidth-liLeft-this.offsetWidth/2))
					{
						...
						oErji.style.right=22+"px";
					}else{
						oErji.style.left=liLeft-sum/2+this.offsetWidth/2-22+"px";
					};
				}else {
					//判断一级li的位置，大于一半,在左侧
					//二级ul的一半大于一级li的位置
		...
		{	
			$(this).removeClass("cu")
			$(this).parents().find('.green').stop(true,true).slideUp(0);
			sum=0;
			if (this.getElementsByTagName("ul")[0]) {
