...
for (var i = 1; i < 12; i++) {
    PREVIEW_DEMODATA.list.push(objClone(PREVIEW_DEMODATA.list[0]));
}
var PREVIEW_CONFIG = {
    site_url: './',
                ...
                for (var i = 0; i < PREVIEW_CONFIG.navs.length; i++) {
                    ...
                    if (typeof PREVIEW_CONFIG.navs[i].childmenu === 'object' && PREVIEW_CONFIG.navs[i].childmenu !== null && PREVIEW_CONFIG.navs[i].childmenu.length > 0) {
                        for (var j = 0; j < PREVIEW_CONFIG.navs[i].childmenu.length; j++) {
                            if (PREVIEW_CONFIG.navs[i].childmenu[j].link == location_link)
                                formatList(PREVIEW_CONFIG.navs[i].childmenu[j]);
                        }
                    }
                }
            }
...
function formatListsData(key, listkey) {
    if (typeof PREVIEW_CONFIG[key] !== 'object' || PREVIEW_CONFIG[key] <= 0)
        return false;
    for (var i = 0; i < PREVIEW_CONFIG[key].length; i++) {
        if (typeof PREVIEW_CONFIG[key][i].type === 'undefined')
            continue;
...
(function () {
    ...
    var buildInFunctions = {expression: {parse: function (s, tree)
            {
                ...
                tree.push({type: 'build-in', name: 'expression', expression: e.tree, params: parseParams(s.slice(e.value.length).replace(/^\s+|\s+$/g, ''))});
                return e.tree;
            }, process: function (node, data)
            {
                ...
                return res;
            }}, operator: {process: function (node, data)
            {
                var params = getActualParamValues(node.params, data);
                var arg1 = params[0];
            ...
            }}, section: {type: 'block', parse: function (params, tree, content)
            {
                var subTree = [];
                var subTreeElse = [];
                tree.push({type: 'build-in', name: 'section', params: params, subTree: subTree, subTreeElse: subTreeElse});
            ...
            }, process: function (node, data)
            {
                ...
                return process(node.subTreeElse, data);
            }}, setfilter: {type: 'block', parseParams: function (paramStr)
            {
                return[parseExpression('__t()|' + paramStr).tree];
            }, parse: function (params, tree, content)
            {
                tree.push({type: 'build-in', name: 'setfilter', params: params, subTree: parse(content, [])});
            }, process: function (node, data)
            {
                ...
                tpl_modifiers = [];
                return s;
            }}, 'for': {type: 'block', parseParams: function (paramStr)
            {
            ...
            }, parse: function (params, tree, content)
            {
                var subTree = [];
                var subTreeElse = [];
                tree.push({type: 'build-in', name: 'for', params: params, subTree: subTree, subTreeElse: subTreeElse});
            ...
            }, process: function (node, data)
            {
                ...
                return s;
            }}, 'if': {type: 'block', parse: function (params, tree, content)
            {
                var subTreeIf = [];
                var subTreeElse = [];
                tree.push({type: 'build-in', name: 'if', params: params, subTreeIf: subTreeIf, subTreeElse: subTreeElse});
            ...
            }}, foreach: {type: 'block', parseParams: function (paramStr)
            {
                var params = {};
            ...
            }, parse: function (params, tree, content)
            {
                var subTree = [];
                var subTreeElse = [];
                tree.push({type: 'build-in', name: 'foreach', params: params, subTree: subTree, subTreeElse: subTreeElse});
            ...
            }, process: function (node, data)
            {
                ...
                return process(node.subTreeElse, data);
            }}, 'function': {type: 'block', parse: function (params, tree, content)
            {
                var subTree = [];
                plugins[trimQuotes(params.name ? params.name : params[0])] = {type: 'function', subTree: subTree, defautParams: params, process: function (params, data)
                    {
                ...
                parse(content, subTree);
            }}, php: {type: 'block', parse: function (params, tree, content) {}}, 'extends': {type: 'function', parse: function (params, tree)
            {
                tree.splice(0, tree.length);
                getTemplate(trimQuotes(params.file ? params.file : params[0]), tree);
            }}, block: {type: 'block', parse: function (params, tree, content)
            {
                tree.push({type: 'build-in', name: 'block', params: params});
            ...
            }}, strip: {type: 'block', parse: function (params, tree, content)
            {
                parse(content.replace(/[ \t]*[\r\n]+[ \t]*/g, ''), tree);
            }}, literal: {type: 'block', parse: function (params, tree, content)
            {
                parseText(content, tree);
            }}, ldelim: {type: 'function', parse: function (params, tree)
            {
                parseText(jSmart.prototype.left_delimiter, tree);
            }}, rdelim: {type: 'function', parse: function (params, tree)
            {
                parseText(jSmart.prototype.right_delimiter, tree);
            }}, 'while': {type: 'block', parse: function (params, tree, content)
            {
                tree.push({type: 'build-in', name: 'while', params: params, subTree: parse(content, [])});
            }, process: function (node, data)
            {
                var s = '';
    ...
    function parseFunc(name, params, tree)
    {
        ...
        tree.push({type: 'plugin', name: '__func', params: params});
        return tree;
    }
    ...
    var tokens = [{re: /^\$([\w@]+)/, parse: function (e, s)
            ...
            }}, {re: /^(true|false)/i, parse: function (e, s)
            {
                parseText(e.token.match(/true/i) ? '1' : '', e.tree);
            }}, {re: /^'([^'\\]*(?:\\.[^'\\]*)*)'/, parse: function (e, s)
            {
            ...
            }}, {re: /^"([^"\\]*(?:\\.[^"\\]*)*)"/, parse: function (e, s)
            {
                ...
                e.tree.push({type: 'plugin', name: '__quoted', params: {__parsed: parse(v, [])}});
                parseText.parseEmbeddedVars = false;
                parseModifiers(s, e);
            }}, {re: /^(\w+)\s*[(]([)]?)/, parse: function (e, s)
            {
                var fnm = RegExp.$1;
                var noArgs = RegExp.$2;
            ...
            }}, {re: /^\s+(eq|ne|neq)\s+/i, parse: function (e, s)
            {
            ...
            }}, {re: /^\s+not\s+/i, parse: function (e, s)
            {
                parseOperator('!', 'pre-unary', 2, e.tree);
            }}, {re: /^\s*(=|\+=|-=|\*=|\/=|%=)\s*/, parse: function (e, s)
            {
            ...
            }}, {re: /^\s+mod\s+/i, parse: function (e, s)
            {
            ...
            {
                parseOperator(RegExp.$1.replace(/<>|&lt;&gt;/, '!=').replace(/&lt;/, '<').replace(/&gt;/, '>'), 'binary', 5, e.tree);
            }}, {re: /^\s+(lt|lte|le|gt|gte|ge)\s+/i, parse: function (e, s)
            {
                var op = RegExp.$1.replace(/l(t)?e/, '<=').replace(/lt/, '<').replace(/g(t)?e/, '>=').replace(/gt/, '>');
                parseOperator(op, 'binary', 5, e.tree);
            }}, {re: /^\s+(is\s+(not\s+)?div\s+by)\s+/i, parse: function (e, s)
            {
                parseOperator(RegExp.$2 ? 'div_not' : 'div', 'binary', 7, e.tree);
            }}, {re: /^\s+is\s+(not\s+)?(even|odd)(\s+by\s+)?\s*/i, parse: function (e, s)
            {
            ...
            }}, {re: /^\s+and\s+/i, parse: function (e, s)
            {
                parseOperator('&&', 'binary', 11, e.tree);
            }}, {re: /^\s+xor\s+/i, parse: function (e, s)
            {
                parseOperator('xor', 'binary', 12, e.tree);
            }}, {re: /^\s+or\s+/i, parse: function (e, s)
            {
                parseOperator('||', 'binary', 13, e.tree);
            }}, {re: /^#(\w+)#/, parse: function (e, s)
            {
        ...
        for (var i = 0; i < tree.length; ++i)
        {
            ...
            } else if (node.type == 'plugin')
            {
                ...
                if (plugin.type == 'block')
                {
                    ...
                    {
                        repeat.value = false;
                        s += plugin.process(getActualParamValues(node.params, data), process(node.subTree, data), data, repeat);
                    }
                } else if (plugin.type == 'function')
                {
    ...
    jSmart = function (tpl)
    {
        ...
        this.smarty = {'smarty': {block: {}, 'break': false, capture: {}, 'continue': false, counter: {}, cycle: {}, foreach: {}, section: {}, now: Math.floor((new Date()).getTime() / 1000), 'const': {}, config: {}, current_dir: '/', template: '', ldelim: jSmart.prototype.left_delimiter, rdelim: jSmart.prototype.right_delimiter, version: '2.13.1'}};
        blocks = this.tree.blocks;
        parse(applyFilters(jSmart.prototype.filters_global.pre, stripComments((new String(tpl ? tpl : '')).replace(/\r\n/g, '\n'))), this.tree);
    };
    ...
    jSmart.prototype.registerPlugin('function', '__quoted', function (params, data)
    {
        return params.join('');
    });
    jSmart.prototype.registerPlugin('function', 'append', function (params, data)
    {
    ...
    jSmart.prototype.registerPlugin('function', 'assign', function (params, data)
    {
        assignVar(params.__get('var', null, 0), params.__get('value', null, 1), data);
        return'';
    });
    jSmart.prototype.registerPlugin('function', 'break', function (params, data)
    {
        data.smarty['break'] = true;
        return'';
    });
    jSmart.prototype.registerPlugin('function', 'call', function (params, data)
    {
    ...
    jSmart.prototype.registerPlugin('block', 'capture', function (params, content, data, repeat)
    {
        if (content)
        {
    ...
    jSmart.prototype.registerPlugin('function', 'continue', function (params, data)
    {
        data.smarty['continue'] = true;
        return'';
    });
    jSmart.prototype.registerPlugin('function', 'counter', function (params, data)
    {
    ...
    jSmart.prototype.registerPlugin('function', 'html_checkboxes', function (params, data) {
        var type = params.__get('type', 'checkbox'), name = params.__get('name', type), realName = params.__get('name', type), values = params.__get('values', params.options), output = params.__get('options', []), useName = ('options'in params), selected = params.__get('selected', false), separator = params.__get('separator', ''), labels = Boolean(params.__get('labels', true)), label_ids = Boolean(params.__get('label_ids', false)), p, res = [], i = 0, s = '', value, id;
        if (type == 'checkbox') {
            name += '[]';
        }
        if (!useName) {
    ...
    jSmart.prototype.registerPlugin('function', 'html_options', function (params, data)
    {
    ...
    jSmart.prototype.registerPlugin('function', 'html_select_date', function (params, data)
    {
    ...
    jSmart.prototype.registerPlugin('function', 'html_table', function (params, data)
    {
        var loop = [];
        var p;
    ...
    jSmart.prototype.registerPlugin('function', 'include_javascript', function (params, data)
    {
        var file = params.__get('file', null, 0);
    ...
    jSmart.prototype.registerPlugin('function', 'include_php', function (params, data)
    {
    ...
    jSmart.prototype.registerPlugin('block', 'javascript', function (params, content, data, repeat)
    {
        data['$this'] = data;
    ...
    jSmart.prototype.registerPlugin('function', 'config_load', function (params, data)
    {
    ...
    jSmart.prototype.registerPlugin('block', 'nocache', function (params, content, data, repeat)
    {
        return content;
    });
    jSmart.prototype.registerPlugin('block', 'textformat', function (params, content, data, repeat)
    {
        if (!content) {
            return'';
        }
    ...
    jSmart.prototype.registerPlugin('modifier', 'capitalize', function (s, withDigits)
    {
    ...
    jSmart.prototype.registerPlugin('modifier', 'count', function (v, recursive)
    {
    ...
    jSmart.prototype.registerPlugin('modifier', 'count_characters', function (s, includeWhitespaces)
    {
    ...
    jSmart.prototype.registerPlugin('modifier', 'date_format', function (s, fmt, defaultDate)
    {
        return jSmart.prototype.PHPJS('strftime', 'date_format').strftime(fmt ? fmt : '%b %e, %Y', jSmart.prototype.makeTimeStamp(s ? s : defaultDate));
    });
    jSmart.prototype.registerPlugin('modifier', 'defaultValue', function (s, value)
    {
        return(s && s != 'null' && s != 'undefined') ? s : (value ? value : '');
    });
    jSmart.prototype.registerPlugin('modifier', 'unescape', function (s, esc_type, char_set)
    {
        s = new String(s);
    ...
    jSmart.prototype.registerPlugin('modifier', 'escape', function (s, esc_type, char_set, double_encode)
    {
        s = new String(s);
        esc_type = esc_type || 'html';
        char_set = char_set || 'UTF-8';
    ...
    jSmart.prototype.registerPlugin('modifier', 'indent', function (s, repeat, indentWith)
    {
    ...
    jSmart.prototype.registerPlugin('modifier', 'replace', function (s, search, replaceWith)
    {
        if (!search)
        {
            return s;
    ...
    jSmart.prototype.registerPlugin('modifier', 'noprint', function (s)
    {
        return'';
    });
    jSmart.prototype.registerPlugin('modifier', 'string_format', function (s, fmt)
    {
        return jSmart.prototype.PHPJS('sprintf', 'string_format').sprintf(fmt, s);
    });
    jSmart.prototype.registerPlugin('modifier', 'strip', function (s, replaceWith)
    {
        replaceWith = replaceWith ? replaceWith : ' ';
        return(new String(s)).replace(/[\s]+/g, replaceWith);
    });
    jSmart.prototype.registerPlugin('modifier', 'strip_tags', function (s, addSpace)
    {
        addSpace = (addSpace == null) ? true : addSpace;
        return(new String(s)).replace(/<[^>]*?>/g, addSpace ? ' ' : '');
    });
    jSmart.prototype.registerPlugin('modifier', 'truncate', function (s, length, etc, breakWords, middle)
    {
    ...
    jSmart.prototype.registerPlugin('modifier', 'wordwrap', function (s, width, wrapWith, breakWords)
    {
        width = width || 80;
        wrapWith = wrapWith || '\n';
        var lines = s.split('\n');
...
function strftime(fmt, timestamp) {
    ...
    var _date = ((typeof timestamp === 'undefined') ? new Date() : (typeof timestamp === 'object') ? new Date(timestamp) : new Date(timestamp * 1000));
    var _aggregates = {c: 'locale', D: '%m/%d/%y', F: '%y-%m-%d', h: '%b', n: '\n', r: 'locale', R: '%H:%M', t: '\t', T: '%H:%M:%S', x: 'locale', X: 'locale'};
...
function setlocale(category, locale) {
    ...
    if (!phpjs.locales) {
        phpjs.locales = {};
        phpjs.locales.en = {'LC_COLLATE': function (str1, str2) {
        ...
        phpjs.locales.C.LC_MONETARY = {int_curr_symbol: '', currency_symbol: '', mon_decimal_point: '', mon_thousands_sep: '', mon_grouping: [], p_cs_precedes: 127, p_sep_by_space: 127, n_cs_precedes: 127, n_sep_by_space: 127, p_sign_posn: 127, n_sign_posn: 127, positive_sign: '', negative_sign: '', int_frac_digits: 127, frac_digits: 127};
        phpjs.locales.C.LC_NUMERIC = {decimal_point: '.', thousands_sep: '', grouping: []};
    ...
    if (!phpjs.localeCategories) {
        phpjs.localeCategories = {'LC_COLLATE': phpjs.locale, 'LC_CTYPE': phpjs.locale, 'LC_MONETARY': phpjs.locale, 'LC_NUMERIC': phpjs.locale, 'LC_TIME': phpjs.locale, 'LC_MESSAGES': phpjs.locale}
    }
...
function evalScripts(text) {
    ...
    while ((script = regexp.exec(text)))
        if (/src\s*=\s*"/.test(script[1]))
            srcScripts.push(script[2]);
        else
...
jSmart.prototype.getTemplate = function (file) {
    ...
    var getFileData = function (jsonfiledata) {
        try {
            var jsonfiledata = jsonfiledata ? eval('(' + jsonfiledata + ')') : {};
        } catch (e) {
            throw new Error('【' + jsonfile + '】文件内容不符合JSON格式！\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json');
        }
...
jSmart.prototype.registerFilter('variable', function (s, a) {
    if (typeof s === 'undefined')
        console.warn('Tips: 请检查html文件，有变量引用不存在！');
    return s;
});
// Smarty Plugins
...
function saveConfig(index, data) {
    ...
    for (var k in data) {
        ...
        for (var dk in data[k])
            if (!/^(value|type|description|config)$/.test(dk))
                checkDataK = false;
        ...
        switch (data[k].type) {
            case 'text':
                if (typeof slimming[k] !== 'string')
                    throw new Error('json文件中type为【text】格式的value值必须为【字符串】\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json#typetext');
                break;
            case 'textarea':
                if (typeof slimming[k] !== 'string')
                    throw new Error('json文件中type为【textarea】格式的value值必须为【字符串】\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json#typetextarea');
            ...
            case 'image':
                ...
                break;
            case 'images':
                if (typeof slimming[k] !== 'object' || typeof slimming[k].length === 'undefined')
                    throw new Error('json文件中type为【images】格式不正确！\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json#typeimages');
    ...
    AJAX('config.ini', function (config) {
        ...
        if (typeof TEMPLATE_CONFIG.Debug === 'object') {
            ...
            for (var k in TEMPLATE_CONFIG.Debug) {
                try {
                    debugJSON = eval('(' + TEMPLATE_CONFIG.Debug[k] + ')');
                } catch (e) {
                    debugJSON = false;
                }
        ...
        function dealData() {
                        ...
                        function jsJQueryOnload() {
                            ...
                            jsJsoneditor.onload = jsJsoneditor.onreadystatechange = function () {
                                ...
                                $('#json-show').val(JSON.stringify(PREVIEW_CONFIG_edit, null, "\t")).change(function () {
                                    ...
                                    if (val) {
                                        try {
                                            PREVIEW_CONFIG_edit = JSON.parse(val);
                                        } catch (e) {
                                            alert('json格式不正确！' + e);
                                        }
                                    }
            ...
            AJAX('global.json', function (globalData) {
                try {
                    globalData = eval('(' + globalData + ')');
                } catch (e) {
                    throw new Error('【global.json】文件内容不符合JSON格式！\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json');
                }
            ...
            function loadOwnJson(jsonfiledata) {
                if (TEMPLATE_CONFIG.Config.type == 1) {
                    try {
                        jsonfiledata = eval('(' + jsonfiledata + ')');
                    } catch (e) {
                        throw new Error('【' + jsonfile + '】文件内容不符合JSON格式！\r\n详情参见：http://pme.eexx.me/doku.php?id=ued:template:json');
                    }
