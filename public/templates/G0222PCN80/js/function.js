$(document).ready(function() {
	...
	(function(){
	    ...
	    block.css({"left":init.position().left + 31}); //初始化滑块位置
	    // nav.hover(function(){},function(){ block.animate({"left":init.position().left + 31},100); }); //移出导航滑块返回
	    ...
	    jQuery(".nav").slide({ 
	            ...
	            startFun:function(i,c,s,tit){ //控制当前滑块位置
	                block.animate({"left":tit.eq(i).position().left + 31},100);
	            }
	        });
	})()

	$(".nav .sub li:first-child").css({"paddingTop":"22px"});
	$(".nav .sub li:last-child").css({"paddingBottom":"22px"});
	...
	// 内页pro页面
	$(".pro li:nth-child(3n-1)").css({marginRight:"17px",marginLeft:"17px"});
