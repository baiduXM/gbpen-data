// update 2015.5.7 16:47

// 以下为模拟PHP输出数据，变量说明参见：http://pme/wiki/doku.php?id=ued:temple
...
var PREVIEW_CONFIG = {
	...
	page_links: {
		...
		per_page: 12,
		current_page: 6,
		first_link: 'list-text.html?p=1',
		prev_link: 'list-text.html?p=5',
		next_link: 'list-image.html?p=6',
		last_link: 'list-image.html?p=99',
		nears_link: {
			4: 'list-imagetext.html?p=4',
			5: 'list-imagetext.html?p=5',
			6: '#',
			7: 'list-imagetext.html?p=7',
			8: 'list-imagetext.html?p=8'
		}
	},
	mIndexCats: [{
		name: '新闻列表',
		...
		image: 'http://chanpin.xm12t.com.cn/images/cat_01.jpg',
		description: '这是栏目的一段描述...',
		list: objClone(PREVIEW_DEMODATA.list),
		type: 1,
		showtype: 1
	}, {
		name: '企业动态',
...
var changeConfigSource = function(cat) {
	switch (true) {
		case cat.indexOf('旅游')!=-1:
			...
			if (typeof PREVIEW_CONFIG.list !== 'undefined') {
			...
			if (typeof PREVIEW_CONFIG.related !== 'undefined') {
				PREVIEW_CONFIG.related = objClone(PREVIEW_DEMODATA.list);
			}
			if (typeof PREVIEW_CONFIG.article !== 'undefined') {
			...
			if (typeof PREVIEW_CONFIG.mIndexCats !== 'undefined') {
...
// 数据逻辑化
formatListsData('mIndexCats', 'list');
...
!function() {
	...
	if (/\/(list-[a-z]+|searchresult)\.htm/.test(location.href)) {
		...
		var setCurrent = function(navs, key) {
			var cur = false;
...
function AJAX(url, callback, fail_callback) {
	...
	function sendAjaxRequest(url, callback, fail_callback) {
		...
		try {
			...
			throw new Error('未配置好实时预览模板！详情参见：http://pme/wiki/doku.php?id=ued:template#%E5%AE%9E%E6%97%B6%E9%A2%84%E8%A7%88');
		}
	}
	...
	var getFileData = function(jsonfiledata) {
		...
		AJAX(file, function(filedata) {
			...
			while (newNode.childNodes.length > 0) {
				_includeParent.insertBefore(newNode.childNodes[0], _include);
			}
			_includeParent.removeChild(_include);
			var tpl = new jSmart(_includeParent.innerHTML);
			_includeParent.innerHTML = tpl.fetch(PREVIEW_CONFIG);
		});
	};
...
jSmart.prototype.registerFilter('variable', function(s,a){
    if (typeof s === 'undefined') console.log('Tips: 请检查html文件，有变量引用不存在！');
	return s;
});
	...
	for (var k in data) {
		...
		switch (data[k].type) {
			...
			case 'images':
				...
				break;
			case 'list':
				if (typeof data[k].config === 'object' && (typeof data[k].config.limit === 'undefined' || typeof data[k].config.star_only === 'undefined'))
					throw new Error('json文件中type为【list】格式的【config】值必须包含【limit、star_only】元素\r\n详情参见：http://pme/wiki/doku.php?id=ued:template:json#typelist');
				slimming[k] = {
					name: '栏目B',
					en_name: 'Nav B',
					image: 'http://chanpin.xm12t.com.cn/images/cat_01.jpg',
				...
				} else if (data[k].config !== 'object' || typeof data[k].config.filter === 'undefined' && data[k].config.filter == 'list') {
				...
				break;
			case 'navs':
				if (TEMPLATE_CONFIG.Config.type == 1) throw new Error('PC模板的json文件中没有type为【navs】的变量！\r\n如果你现在制作的是手机模板，请修改config.ini文件对应参数。详情参见：http://pme/wiki/doku.php?id=ued:template:config#config_%E6%A8%A1%E6%9D%BF%E9%85%8D%E7%BD%AE%E9%83%A8%E5%88%86');
	...
	AJAX('config.ini', function(config) {
				...
				if (/^[_a-zA-Z]+[A-Za-z0-9_]*$/.test(k)) {
					...
					try {
						typeof debugJSON === 'object' ?
							eval('PREVIEW_CONFIG.'+k+' = (' + TEMPLATE_CONFIG.Debug[k] + ');console.log(11)') :
		...
		AJAX('global.json', function(globalData) {
			...
			if (TEMPLATE_CONFIG.Config.type == 1) {
				...
				throw new Error('【global.json】文件不存在！文件格式说明详见：http://pme/wiki/doku.php?id=ued:template:json');
		});
		...
		function loadOwnJson(jsonfiledata) {
			...
			if (!DEBUG) {
				if (typeof document.querySelectorAll !== 'undefined') {
					for (var links = document.querySelectorAll("link[href]:not([href^='{$site_url}'])"), i = links.length; i--;) {
						if (!/^http[s]?:\/\//.test(links[i].getAttribute("href")) && !/^\{\$.*\}$/.test(links[i].getAttribute("href"))) {
							throw new Error('所有的css引用link都必须加入路径变量【{$site_url}】\r\n详情参见：http://pme/wiki/doku.php?id=ued:template#%E6%A8%A1%E6%9D%BFjs_css%E5%BC%95%E7%94%A8');
						}
					}
				...
				if (document.head.innerHTML.indexOf('{$title}') == -1) throw new Error('所有页面的head内必须加入【{$title}】\r\n详情参见：http://pme/wiki/doku.php?id=ued:template:index#title_%E9%A1%B5%E9%9D%A2%E6%A0%87%E9%A2%98');
				if (document.head.innerHTML.indexOf('{$keywords}') == -1) throw new Error('所有页面的head内必须加入【{$keywords}】\r\n详情参见：http://pme/wiki/doku.php?id=ued:template:index#keywords_%E9%A1%B5%E9%9D%A2%E5%85%B3%E9%94%AE%E8%AF%8D');
				if (document.head.innerHTML.indexOf('{$description}') == -1) throw new Error('所有页面的head内必须加入【{$description}】\r\n详情参见：http://pme/wiki/doku.php?id=ued:template:index#description_%E9%A1%B5%E9%9D%A2%E6%8F%8F%E8%BF%B0');
				if (filename == 'index') {
					if (!/^{\$headscript}/.test(document.body.innerHTML)) {
						throw new Error('index.html的head最后必须加入【{$headscript}】\r\n详情参见：http://pme/wiki/doku.php?id=ued:template:index#headscript_%E5%A4%B4%E9%83%A8%E8%84%9A%E6%9C%AC');
					}
					if (document.head.innerHTML.indexOf('{$site_another_url}') == -1) throw new Error('index.html的head顶端必须加入【{$site_another_url}】判断访问设备跳转\r\n详情参见：http://pme/wiki/doku.php?id=ued:template:index#site_another_url_%E7%BD%91%E7%AB%99%E5%8F%A6%E4%B8%80%E5%9F%9F%E5%90%8D%E5%9C%B0%E5%9D%80');
				}
				// 延迟校验模板格式
				setTimeout(function() {
					if (document.body.innerHTML.indexOf('<!-- 这里将会输出底部脚本代码 -->') == -1) {
						throw new Error('所有页面的最后（可以在_footer.html中）必须加入【{$footscript}】\r\n详情参见：http://pme/wiki/doku.php?id=ued:template:footer#footscript_%E5%BA%95%E9%83%A8%E8%84%9A%E6%9C%AC');
					}
					if (document.body.innerHTML.indexOf('copyright 2014-2015') == -1) {
						throw new Error('所有页面的最后（可以在_footer.html中）必须加入【{$footprint}】\r\n详情参见：http://pme/wiki/doku.php?id=ued:template:footer#footprint_%E5%BA%95%E9%83%A8%E7%89%88%E6%9D%83');
					}
				}, 3000);
			}
			
			// jSmarty
			console.log(PREVIEW_CONFIG);
			...
			// 执行js
			var script_srcs = [];
			for (var scripts = document.getElementsByTagName("script"),i=0; i<scripts.length;) {
				if (scripts[i].getAttribute("src") && scripts[i].getAttribute("src").indexOf('preview.js') == -1) {
		...
		if (status == 404)
			throw new Error('【config.ini】文件不存在！文件格式说明详见：http://pme/wiki/doku.php?id=ued:template:config');
	});
});